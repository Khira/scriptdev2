...
bool AddItem(Player * pPlayer, uint32 IdItem, uint32 Count /*= 1*/)
{
    ...
    uint8 Msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, Dest, IdItem, Count, NULL);
    if(Msg == EQUIP_ERR_OK)
    {
        Item * _Item = pPlayer->StoreNewItem(Dest, IdItem, Count, true);
        pPlayer->SendNewItem(_Item, Count, true, false, false);
        return true;
    }
    return false;
}
