...
namespace
{
    ...
    {
        return (SuccesProbability ? rand() % 100 <= SuccesProbability : false);
    }
    ...
    void ActionAchat(Player * const pPlayer, Creature * const pCreature, const uint32 &id)
    {
        ...
        if (GagneJet(pRec->m_SuccesProbability))
        {
            ...
            {
                pCreature->MonsterWhisper("Vous n'avez pas assez de place dans votre inventaire.", pPlayer->GetGUID());
                return;
            }
        ...
        {
            pCreature->MonsterWhisper("C'etait un risque a prendre. Vous avez perdu. Desole.",
                                      pPlayer->GetGUID());
        }
